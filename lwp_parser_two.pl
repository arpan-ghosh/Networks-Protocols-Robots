...
my @urls = ( ); #links that are not self-referencing and non local 
...
sub call_back {
    ...
    # here.
    #
    push @urls, $atrribs { 'href' } if $tag eq "a";
...
# to remove self referncing and local links
while ((my $key, my $val) = each @urls) {
	if (($key =~ /$base#.*/) || ($key != /\.$domain/)) {
		delete $urls{$key};
	}
}
